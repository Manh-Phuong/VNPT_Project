...
function ProductDetail() {
    ...
    const handleOnUpdateProduct = () => {
        ...
        navigate(`/sell_product/update/${variantId}`);
    };
    ...
    const fetchData = async () => {
        try {
            const res = await ProductServices.getProductDetail(id, token);
            console.log('===================== ', res);
            ...
            if (variantId) {
                for (let i = 0; i < res?.data.variants.length; i++) {
                    if (res?.data.variants[i].id == variantId) {
                        setVariantIdParam(variantId);
                        setVariant(res?.data.variants[i]);
                        setActive(res?.data.variants[i].id);
                        return;
                    }
                }
            ...
            setActive(res?.data.variants[0].id);
        } catch (error) {
            console.log(error);
            return error;
        }
                        ...
                        <div className={cx('personalInfoContent')}>
                            <div className={cx('personalInfoContentRow')}>
                                ...
                                <div className={cx('personalInfoContentCol')}>
                                    ...
                                    <div className={cx('personalInfoValue')}>
                                        : {product.label ? <span>{product.label}</span> : <span>---</span>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={cx('personalInfoContentRow')}>
                                <div className={cx('personalInfoContentCol')}>
                                    ...
                                    <div className={cx('personalInfoValue')}>
                                        :{' '}
                                        {product.quantity ? (
                                            <span>{numeral(product.quantity).format(0, 0)}</span>
                                        ) : (
                                            <span>0</span>
                                        )}
                                    </div>
                            ...
                            <div className={cx('personalInfoContentRow')}>
                                <div className={cx('personalInfoContentCol')}>
                                    ...
                                    <div className={cx('personalInfoValue')}>
                                        :{' '}
                                        {product.createdAt ? (
                                            <span>{formattedDate(product.createdAt)}</span>
                                        ) : (
                                            <span>---</span>
                                        )}
                                    </div>
                    ...
                    <div className={cx('variant-content')}>
                            ...
                            <div className={cx('variant-list-content')}>
                                {product.variants
                                    ? product.variants.map((variant, index) => (
                                          <Link
                                              // to={generatePath(routes.productDetail2, {
                                              //     id: product.id,
                                              //     variantId: variant.id,
                                              // })}
                                              key={index}
                                          >
                                              <div
                                                  ...
                                                  className={cx(
                                                      'variant-item',
                                                      variant.id == active || variantIdParam == variant.id
                                                          ? 'active'
                                                          : '',
                                                  )}
                                                  onClick={() => handleOnClickVariant(variant)}
                                              >
                                                  ...
                                                  <div className={cx('variant-info')}>
                                                      ...
                                                      <span className={cx('variant-quantity')}>
                                                          Tồn kho: {numeral(variant.quantity).format(0, 0)}
                                                      </span>
                                                  </div>
                                                  {variant.quantity === 0 ? (
                        ...
                        <div className={cx('variant-detail')}>
                            <div className={cx('variant-info-detail')}>
                                <div className={cx('variant-info-detail-title')}>Thông tin chi tiết phiên bản</div>
                                        ...
                                        <div className={cx('personalInfoContentRow')}>
                                            <div className={cx('personalInfoContentCol')}>
                                                ...
                                                <div className={cx('personalInfoValue')}>
                                                    :{' '}
                                                    {variant.createdAt ? (
                                                        <span>{formattedDate(variant.createdAt)}</span>
                                                    ) : (
                                                        <span>---</span>
                                                    )}
                                                </div>
                            ...
                            <div className={cx('variant-price')}>
                                <div className={cx('variant-price-title')}>Giá sản phẩm</div>
                                    ...
                                    <div className={cx('personalInfoContentRow')}>
                                        <div className={cx('personalInfoContentCol')}>
                                            ...
                                            <div className={cx('personalInfoValue')}>
                                                :{' '}
                                                {variant.retailPrice ? (
                                                    <span>{numeral(variant.retailPrice).format(0, 0)}</span>
                                                ) : (
                                                    <span>0</span>
                                                )}
                                            </div>
                                        ...
                                        <div className={cx('personalInfoContentCol')}>
                                            ...
                                            <div className={cx('personalInfoValue')}>
                                                ...
                                                {variant.wholeSalePrice ? (
                                                    <span>{numeral(variant.wholeSalePrice).format(0, 0)}</span>
                                                ) : (
                                                    <span>0</span>
                                                )}
                                            </div>
                                    ...
                                    <div className={cx('personalInfoContentRow')}>
                                        <div className={cx('personalInfoContentCol')}>
                                            ...
                                            <div className={cx('personalInfoValue')}>
                                                :{' '}
                                                {variant.importPrice ? (
                                                    <span>{numeral(variant.importPrice).format(0, 0)}</span>
                                                ) : (
                                                    <span>0</span>
                                                )}
                                            </div>
